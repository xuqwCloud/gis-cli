...
module.exports = async () => {
    //打印欢迎界面
    clear();
    const data = await figlet('Welcome to GIS-CLI');
    log(data);
    ...
    inquirer
        ...
        .then(async (answer) => {
            if (answer.state === 'vue') {
                ...
                process.start();
                createProject(path.join(__dirname, './../templates/vuemaptempl'), answer.name, copy);
...
=====================
                ...
                const process = ora(`项目创建中 Loading…………`);
                process.start();
                createProject(path.join(__dirname, './../templates/reactmaptempl'), answer.name, copy);
                process.text = `项目创建完成 ->->->-> Successed！`;
                process.succeed();
